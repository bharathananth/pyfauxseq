...
def test_params(n_genes, reps, rhy_frac, depth, min_A_effect, lib_size_var):
    data = generate_diffrhythmic_rnaseq(n_genes=n_genes, reps=reps, rhy_frac=rhy_frac,
                                    min_A_effect=min_A_effect, depth=depth,
                                    lib_size_var=lib_size_var)
    assert data["params"]["category"].isin(["loss", "gain", "change",
                                            "same", np.nan]).all()
    assert data["params"]["id"].isin(data["counts"].index.to_list()).all()
    assert (data["params"]["A_ctrl"].dropna() >= np.log2(1 + min_A_effect)).all()
    assert (data["params"]["A_expt"].dropna() >= np.log2(1 + min_A_effect)).all()
    assert (data["params"]["phi_ctrl"].dropna() >=0).all() and \
                        (data["params"]["phi_ctrl"].dropna() <= 2*np.pi).all()
    assert (data["params"]["phi_expt"].dropna() >=0).all() and \
                        (data["params"]["phi_expt"].dropna() <= 2*np.pi).all()
